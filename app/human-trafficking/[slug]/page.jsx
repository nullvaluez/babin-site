...
export async function generateStaticParams() {
  return humanTraffickingServices.map((s) => ({ slug: s.slug }));
}
...
export default async function HumanTraffickingServicePage({ params }) {
  const { slug } = await params;
  const service = getServiceBySlug(CATEGORY_SLUG, slug);
  if (!service) notFound();
  ...
  const breadcrumbJsonLd = generateBreadcrumbJsonLd([
    { name: "Home", href: "/" },
    { name: "Human Trafficking", href: "/human-trafficking" },
    { name: service.title },
  ]);
